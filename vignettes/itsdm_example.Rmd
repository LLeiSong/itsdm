...
#> 
#> 
#> row [520] - suspicious column: [bio19] - suspicious value: [36.00]
#> 	distribution: 97.222% >= 509.00 - [mean: 614.97] - [sd: 40.52] - [norm. obs: 35]
#> 	given:
#> 		[bio17] > [2.00] (value: 27.00)
#> 		[bio5] > [35.53] (value: 36.72)
#> 
#> 
#> row [12] - suspicious column: [wdpa] - suspicious value: [1.00]
#> 	distribution: 96.667% >= 2.00 - [mean: 2.00] - [sd: 0.00] - [norm. obs: 29]
#> 	given:
#> 		[bio1] > [20.48] (value: 28.85)
#> 		[bio2] > [12.75] (value: 13.08)
#> 		[bio15] > [123.23] (value: 124.89)
plot(occ_outliers)
```
...
#> bio1   1.00  0.02 -0.22  0.64  0.06 -0.39 -0.41  0.28
#> bio2   0.02  1.00 -0.43 -0.66 -0.55 -0.46 -0.34 -0.29
#> bio3  -0.22 -0.43  1.00  0.44  0.33  0.44  0.23  0.22
#> bio6   0.64 -0.66  0.44  1.00  0.46  0.11 -0.09  0.48
...
#> bio19  0.28 -0.29  0.22  0.48  0.54  0.30 -0.07  1.00
...
#> CVI with 0.25 threshold:      0.013
#> CVI with 0.5 threshold:       0.132
#> CVI with 0.75 threshold:      0.576
#> CBI:                          0.602
#> AUC (ratio)                   0.873
#> ===================================
...
#> AUC:                          0.862
#> Similarity indices:
#> Jaccard's similarity index:   0.644
#> Sørensen's similarity index:  0.784
#> Overprediction rate:          0.179
#> Underprediction rate:         0.251
...
#> [Training dataset]:
#> bio12     With only: ################################## 0.764
#>           Without  : ############################################ 0.975
#> bio18     With only: ############################ 0.617
#>           Without  : ############################################ 0.983
#> landcover With only: ########################### 0.604
#>           Without  : ############################################ 0.987
#> bio3      With only: ########################### 0.603
#>           Without  : ############################################ 0.978
#> bio6      With only: ######################## 0.533
#>           Without  : ############################################ 0.982
#> bio19     With only: ####################### 0.506
#>           Without  : ########################################### 0.966
#> bio1      With only: #################### 0.454
#>           Without  : ############################################ 0.982
#> wdpa      With only: #################### 0.453
#>           Without  : ############################################ 0.986
#> bio14     With only: ################# 0.368
#>           Without  : ############################################ 0.979
#> bio2      With only: ############## 0.321
#>           Without  : ############################################ 0.985
#> [Test dataset]:
#> bio12     With only: ################################### 0.78
#>           Without  : ############################################ 0.972
#> bio3      With only: ########################### 0.6
#>           Without  : ############################################ 0.977
#> landcover With only: ########################## 0.589
#>           Without  : ############################################ 0.988
#> bio18     With only: ########################## 0.575
#>           Without  : ############################################ 0.985
#> bio19     With only: ######################## 0.535
#>           Without  : ########################################### 0.964
#> bio6      With only: ###################### 0.48
#>           Without  : ############################################ 0.982
#> wdpa      With only: #################### 0.438
#>           Without  : ############################################ 0.985
#> bio1      With only: ################## 0.407
#>           Without  : ############################################ 0.98
#> bio14     With only: ################ 0.363
#>           Without  : ############################################ 0.98
#> bio2      With only: ########### 0.252
#>           Without  : ############################################ 0.986
#> ======================================================================
#> Jackknife test
#> Based on AUC ratio (Max value of traing and test are 0.877 and 0.873)
#> [Training dataset]:
#> bio12     With only: ###################################### 0.843
#>           Without  : ###################################### 0.845
#> bio18     With only: #################################### 0.799
#>           Without  : ####################################### 0.869
#> bio3      With only: ################################## 0.763
#>           Without  : ######################################## 0.881
#> bio6      With only: ################################# 0.724
#>           Without  : ####################################### 0.866
#> bio1      With only: ############################### 0.698
#>           Without  : ######################################## 0.881
#> wdpa      With only: ############################### 0.679
#>           Without  : ####################################### 0.864
#> landcover With only: ############################## 0.677
#>           Without  : ######################################## 0.884
#> bio2      With only: ############################# 0.654
#>           Without  : ######################################## 0.878
#> bio19     With only: ######################## 0.528
#>           Without  : ######################################## 0.88
#> bio14     With only: ############## 0.312
#>           Without  : ######################################## 0.891
#> [Test dataset]:
#> bio12     With only: ##################################### 0.824
#>           Without  : ###################################### 0.841
#> bio18     With only: ################################## 0.758
#>           Without  : ####################################### 0.865
#> bio3      With only: ################################# 0.732
#>           Without  : ####################################### 0.875
#> landcover With only: ############################### 0.689
#>           Without  : ####################################### 0.873
#> bio1      With only: ############################### 0.684
#>           Without  : ####################################### 0.875
#> bio6      With only: ############################## 0.674
#>           Without  : ####################################### 0.862
#> wdpa      With only: ############################## 0.666
#>           Without  : ####################################### 0.858
#> bio2      With only: ########################### 0.591
#>           Without  : ####################################### 0.873
#> bio19     With only: ######################## 0.525
#>           Without  : ####################################### 0.875
#> bio14     With only: ############## 0.31
#>           Without  : ######################################## 0.884
#> ======================================================================
...
#> Error in plot.VariableContribution(var_contrib_outliers, plot_each_obs = T): Too many observations in VariableContribution to plot separately. 
#> Consider to use less observations in VariableContribution or set plot_each_obs to FALSE.
```

According to feature response curves and this figure, some detected outliers could be removed from training. For example, some features have a strongly negative contribution to observations No.1, No.6, No.8, No.10, and No.15 and they could be excluded.
