...
Even though occurrence dataset obtained from GBIF have high quality and its API provide general options to do some screening. There are still some disturbances contained in occurrence. As a complement，we do extra steps to clean the occurrence data. The steps include:
...
## Split occurrence to train and test
...
Now take a look at response curves.
...
plot(it_sdm$variable_dependence, target_var = c('bio1', 'bio12', 'bio18'),
     related_var = 'bio6', smooth_span = 0.7)
plot(it_sdm$variable_dependence, target_var = c('bio1', 'bio6', 'bio12', 'wdpa'),
     related_var = 'landcover', smooth_span = 0)
```

## Analyze variable contribution

Sometimes, we are interested in some observations, for instance, the outliers detected in the beginning steps. 
...
var_contrib_outliers <- variable_contrib(
  ...
  occ_outliers$outliers %>% st_drop_geometry())
...
Let's take a look at general variable contribution.
